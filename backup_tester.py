...
class Tester:
    ...
    def register_all(self):
        """Register all 4 sensors upfront. Enforce P10 on server side."""
    ...
    def _register_once(self, sensor_type, sensor_id):
        """One-shot register for a given sensor_id."""
        msg = {
            "type": "register",
    ...
    def _rx_loop(self):
        ...
        while self._rx_running:
            ...
            if mtype == "request_resend":
                ...
                # prefer last good, fallback to pending
                ...
                if not token or not stype:
                    continue
                # QUIET STOP: ak auto nie je spustene, odpovedaj hned
                if not self.sending:
                    ack = {
                        "type": "activity_ack",
                        "sensor_type": stype,
                    ...
                    self.send_json(ack)
                    # nic dalej neriesime, server je spokojny, log je ticho
                    continue

                # UAT4 flow (auto bezi, senzor je umyselne pozastaveny)
    ...
    def inject_bad_frame(self):
        ...
        # take last good if available, else pending; do not generate a new "good"
        base = self.last_sent_ok.get(s_id) or self.pending_last.get(s_id)
        if base is None:
            print("ERROR: no last good frame to corrupt. Send one manual frame or run auto first.")
            return

        bad = dict(base)
        bad["crc32"] = (bad["crc32"] ^ 0x00FFFF00) & 0xFFFFFFFF
    ...
    def uat4_activity_check(self):
        print("\n--- UAT4: ACTIVITY CHECK ---")
        print("Select sensor to pause (server should detect inactivity):")
        for i, (stype, sid) in enumerate(self.sensors, start=1):
            print(f"{i}. {sid} ({stype})")
        choice = input("Select: ").strip()
        try:
    ...
    def menu(self):
        while True:
            ...
            print("7. UAT4: Activity check (pause sensor)")
            print("8. Exit")
